...
Full transitive dependency graph for `ike-doc` 1-chronology-builder-SNAPSHOT, generated from [bom.json](bom.json)[1] (CycloneDX 1.6) at build time. Same SBOM source as the SPDX-grouped [licenses.html](licenses.html)[2] and the curated [built-with.html](built-with.html)[3] — three views of the same data.
...
- As a Maven artifact: pull `ike-doc:​1-chronology-builder-SNAPSHOT` with `<classifier>cyclonedx</classifier><type>json</type>` from Nexus / Maven Central.
